...
//        $result = $this->app->live->getPlaybacks(...array_values($params));
     ...
     * 文本内容安全识别
     */
    public function textContentCheck($text,$openId) {
     ...
     * 文本内容安全识别
     */
    public function imageContentCheck($media_url,$openId) {
        $url = "https://api.weixin.qq.com/wxa/media_check_async?access_token={$this->getAccessToken()['access_token']}";
        $data = [
            'media_url' => $media_url,
            'media_type'=>2,
            'version' => 2,
