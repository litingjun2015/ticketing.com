<template>
	<view class="content font_family">
		<view class="top_box f_z_b f_j">
			...
			<view class="right" @click="go('/pagesB/income/withdrawal')">
				去提现
			</view>
		</view>
		<!-- 查看收入切换 -->
		...
		<!-- 月份收入展示 -->
		<!-- 头部星期 -->
		<view class="" v-if="btn_action==0">
			<view class="weekday f">
				<view class="item" v-for="(item,index) in weekday" :key="index">
					{{item.text}}
				</view>
			</view>
			<!-- 展示日期 -->
			<view class="date_box f">
				<!-- 占位item -->
				<view class="zw_item" v-for="(item,index) in zw_num" :key="index">

				</view>
				<view class="item f_d f_j" v-for="(item,index) in month_arr" :key="index">
					<view class="top">
						{{item.date}}
					</view>
					<view class="bottom">
						￥{{item.money | price_num}}
					</view>
				</view>
			</view>
		</view>
		<!-- 年份收入展示 -->
		<view class="year f" v-else-if="btn_action==1">
			<view class="item f_d f_zj" v-for="(item,index) in year_arr" :key="index">
				<view class="top">
					{{item.date}}月
				</view>
				<view class="bottom">
		...
		<view class="record" v-else>
			<view class="item" v-for="(item,index) in withdraw_lists" :key="index">
				...
				<view class="bottom f_z_b">
					...
					<view class="right">
						{{item.status==1?'待审核':item.status==2?'审核拒绝':item.status==3?'待到账':'已到账'}}
					</view>
				</view>
			</view>
			<view class="empty_top" v-if="withdraw_lists.length==0">
...
<script>
	import { guide_center, income_detail, withdraw_list } from '@/api/user.js'
	export default {
		data() {
			return {
				cut_off: '',
		...
		onReachBottom() {
			if (!this.withdraw_list_nomore && this.btn_action == 2) {
		...
		methods: {
			...
			get_detail(data) {
				income_detail(data).then(res => {
					if (res.code == 1) {
						if (this.btn_action == 0) {
							if (res.data.days_array[0].weekday > 6) {
								this.zw_num = 0
							} else {
								this.zw_num = res.data.days_array[0].weekday * 1
							}
							this.month_arr = res.data.days_array
						} else if (this.btn_action == 1) {
							this.year_arr = res.data.days_array
						}
						this.income_price = res.data.month_sum
					}
				})
			},
			// 获取提现记录
