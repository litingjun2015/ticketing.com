...
<script>
	import { guide_lists } from '@/api/user.js'
	...
	export default {
		...
		methods: {
			...
			go_detail(item) {
				...
				uni.navigateTo({
					url: `/pagesB/order/order_detail?detail=${detail}&travel=${travel}&type=${item.order_type}&obj=${obj2}&status=${item.status}&order_no=${item.order_no}&pay_price=${item.order_amount_total}&time=${item.pays_time}`
				})
			},
			// 获取商家订单列表
			get_list() {
