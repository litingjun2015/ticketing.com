	...
	<view class="content font_family" :style="{paddingTop:menuButtonInfo+'px'}">
		...
		<view class="list">
			<view class="item" v-for="(item,index) in list" :key="index"
				@click="go_detail(`/pagesB/release/detail?id=${item.id}`)">
				<view class="top f">
					<view class="left">
				...
				<view class="bottom f">
					...
					<view class="btn btn3" @click.stop="go_set(`/pagesB/product/product?id=${item.id}`)">
						编辑
					</view>
				</view>
			</view>
...
<script>
	import { product_list, product_status, product_price } from '@/api/user.js'
